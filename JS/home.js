    ...
    if (this.readyState === 4 && this.status === 200) {
      ...
      for (x = 0; x < result.length; x++) {
        ...
        var card =
          ...
          result[x].name +
          "</h5>" +
          "<h6 class='card-subtitle mb-2'><span style='color:" +
          statuscolor +
          "'>" +
          result[x].status +
          "</span> | TYPE : " +
        ...
        card +=
          "<hr><a href='" +
          result[x].link +
          "' class='card-link'>More Details</a>" +
          "</div>" +
          "</div>" +
          "</div>";
      ...
      document.getElementsByClassName("row")[0].innerHTML =
        '<div style="width:100%;border:1px solid #C39BD3;margin-top:15px;padding:10px 0;text-align:center;font-size:1.2rem;font-weight:lighter;color:#C39BD3">Loading!</div>';
    }
  };
...
function searchcars() {
  ...
  if (query === "") {
    ...
    searchresults.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        searchbox.innerHTML =
          '<h3 style="font-weight:lighter">Search Results</h3>' +
          this.responseText;
      }
    };
    ...
    searchresults.setRequestHeader(
      "Content-type",
